...
def main():
    print("Loading data for Striker Agent...")
    ...
    y = df['shot_direction'] # Target: Where will the striker shoot?
    
    X_train, X_test, y_train, y_test = train_test_split(X_encoded, y, test_size=0.2, random_state=42)
    
    print("Training Striker Agent (CatBoost)...")
    model = CatBoostClassifier(iterations=300, depth=6, learning_rate=0.1, verbose=0, random_seed=42)
    model.fit(X_train, y_train)
    
    print(f"Striker Accuracy: {accuracy_score(y_test, model.predict(X_test)):.4f}")
    
    os.makedirs(MODELS_DIR, exist_ok=True)
    joblib.dump(model, os.path.join(MODELS_DIR, "striker_agent_model.joblib"))
    print("✅ Striker Model saved.")
