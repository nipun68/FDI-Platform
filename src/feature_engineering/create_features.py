...
def categorize_placement(y, z):
    """Categorizes shot placement into 6 zones based on StatsBomb end coordinates."""
    if 36 <= y <= 44:
        horizontal = "Center"
    elif y < 36:
        horizontal = "Left"
...
def main():
    """Main pipeline to extract basic spatial features from real StatsBomb data."""
    print("Loading real StatsBomb penalties data...")
    if not os.path.exists(RAW_DATA_PATH):
        print("Error: penalties_raw.csv not found. Run parse_penalties.py first!")
        return
 
    df = pd.read_csv(RAW_DATA_PATH)
    print("Engineering basic spatial features (Distance, Angle, Zone)...")
    ...
    print(f"\n✅ Real data feature engineering complete! Saved to {output_path}")
    print("\n--- Data Preview ---")
    print(df[['player_name', 'is_goal', 'shot_distance', 'shot_angle', 'placement_zone', 'is_shootout']].head())
    print("\nNote: This real data lacks advanced features (speed, GK behavior). Use synthetic data for advanced ML training.")
